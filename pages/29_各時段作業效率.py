...
# ✅ 你要改的：WMS 匯出資料夾（預設）
# =========================================================
DEFAULT_DATA_DIR = r"C:\WMS_EXPORT"  # ← 改成你的WMS匯出資料夾（也可以填 SMB 映射磁碟例如 Z:\WMS）
DEFAULT_PATTERNS = ["*.csv", "*.txt"]
...
.board-sub { font-size: 14px; color:#6b7280; text-align:right; margin-top:6px; }
.board-bar { width: 100%; height: 34px; background: #e5e7eb; border-radius: 4px; overflow: hidden; position: relative;}
.board-bar > .fill { height: 100%; background:#16a34a; width: 0%; }
...
def render_realtime_board(
    ...
    """
    board_df 必含欄位：線別, 開線時間(HH:MM), 每小時分揀量, 已分揀總量
    """
    ...
    # 當小時有效分鐘（12/13=30分；當小時=now_h 才用 now_m 截止）
...
""",
    ...
    for _, r in show.iterrows():
        ...
        # bar 顏色：未達標 → 紅（用 inline style 換色）
...
def main():
    ...
    st.markdown("### ⏱️ 各時段作業效率（資料夾即時看板｜Excel：保留公式＋色塊自動更新；支援舊 Excel）")
    ...
    # =============================
    # Sidebar：資料夾、刷新、目標、截止時間
    # =============================
    with st.sidebar:
        st.markdown("### 設定")

        data_dir = st.text_input("WMS 匯出資料夾（CSV/TXT）", value=DEFAULT_DATA_DIR)
        patterns = st.multiselect("讀取副檔名/樣式", options=DEFAULT_PATTERNS, default=DEFAULT_PATTERNS)
        max_files = st.number_input("讀取最新檔案數", min_value=1, max_value=200, value=20, step=1)
        ...
        if auto_refresh:
            if HAS_AUTOREFRESH:
                ...
                st.warning("未安裝 streamlit-autorefresh，將無法自動刷新（可 pip install streamlit-autorefresh）")
    ...
    # 上傳人員名單（保留原本）
    # =============================
    mem_file = st.file_uploader("② 上傳『人員名單』(CSV/Excel)（用來解析線別/段數/姓名/開始時間）", type=["csv", "xlsx", "xlsm", "xls"])

    if mem_file is None:
        st.info("請先上傳『人員名單』。生產資料會直接從資料夾讀取最新檔案。")
        return
    ...
    with top[0]:
        st.markdown("#### 指定時段計算")
    with top[1]:
        st.caption("（用『線別』匯總：每小時分揀量/已分揀總量）")

    h_from = top[2].selectbox("起", options=list(range(0, 24)), index=min(max(int(now.hour), 0), 23))
    h_to = top[3].selectbox("訖", options=list(range(0, 24)), index=min(max(int(now.hour), 0), 23))
    do_range_calc = top[4].button("計算", use_container_width=True)
    ...
    if not should_run:
        st.caption("（目前結果已是最新；參數/時間或檔案變動會自動更新）")
        return
    ...
    try:
        ...
        if df_raw.empty:
            raise ValueError("資料夾內找不到可讀取的 CSV/TXT（或檔案格式不正確）。")

        df_raw = _norm_cols(df_raw)
        require_columns(df_raw, ["PICKDATE", "LINEID", "ZONEID", "PACKQTY", "Cweight"], "生產資料（資料夾）")
        ...
        st.caption(
            f"資料來源：{data_dir}｜讀取最新 {int(max_files)} 檔｜"
        ...
        if do_range_calc:
            ...
            show_range = show_range.sort_values("線別")
            st.dataframe(show_range, use_container_width=True)

        # =============================
        # ✅ 保留你原本：各線別 heatmap / KPI（放在展開區，避免干擾看板）
        # =============================
